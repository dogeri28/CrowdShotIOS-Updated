...
@objc public class BanubaSdkManager: NSObject {
    ...
    @objc public func loadEffect(_ effectUrl: String, synchronous: Bool = false) -> BNBEffect? {
        ...
        if synchronous {
            ...
            stopRenderLoop()
            effect = renderQueue.sync {
                self.activateContext()
                return effectPlayer?.effectManager()?.load(effectUrl)
            }
            if shouldRestart {
                startRenderLoop()
            }
    ...
    @objc public func setFeatureColor(color: UIColor) {
        var r: CGFloat = 0
        var g: CGFloat = 0
        var b: CGFloat = 0
        var a: CGFloat = 0
        color.getRed(&r, green: &g, blue: &b, alpha: &a)
        self.setFeatureColor(r: Float(r), g: Float(g), b: Float(b), a: Float(a))
    }
    
    /**
     * - parameter r: red channel in range [0, 1].
     * - parameter g: green channel in range [0, 1].
     * - parameter b: blue channel in range [0, 1].
     * - parameter a: alpha channel in range [0, 1].
     */
    @objc public func setFeatureColor(r: Float, g: Float, b: Float, a: Float) {
        self.featureColor = [r, g, b, a]
    }
    ...
    public private(set) var featureColor: [Float]? = nil
    ...
    @objc public func setRenderTarget(layer: CAEAGLLayer, contentMode: RenderContentMode = .resizeAspectFill, playerConfiguration: EffectPlayerConfiguration?) {
        ...
        if let configuration = currentEffectPlayerConfiguration {
            ...
            renderTarget = RenderTarget(
                ...
                contentMode: contentMode,
                renderQueue: renderRunLoop.renderQueue
            )
            renderRunLoop.renderQueue.async { [weak self] in
                self?.surfaceCreated(width: Int32(configuration.renderSize.width), height: Int32(configuration.renderSize.height))
    ...
    private func startRenderLoop() {
        synchronizationQueue.sync {
            let framerate = currentEffectPlayerConfiguration?.preferredRenderFrameRate ??
                EffectPlayerConfiguration.Defaults.defaultFrameRate
...
internal extension BanubaSdkManager {
    ...
    private func drawToContext() -> Bool {
        ...
        renderTarget.presentRenderbuffer { [weak self] in
            self?.delegate?.willPresentFramebuffer(renderSize: renderTarget.renderSize)
        }
        return true
    }
}
...
@objc extension BanubaSdkManager {
    ...
    public func stopEffectPlayer() {
        stopOrientationDetection()
        stopRenderLoop()
        renderRunLoop.renderQueue.async {
        ...
        renderRunLoop.renderQueue.async { [weak self] in
            autoreleasepool {
                ...
                if let imageData = self.effectPlayer?.drawVideoFrame(frameData, timeNs: 0, outputPixelFormat: .rgba) {
                    image = UIImage(
                        rgbaDataNoCopy: imageData as NSData,
        ...
        DispatchQueue.main.asyncAfter(deadline: cameraInitDelay, execute: { [weak self] in
            self?.inputService.initiatePhotoCapture(
                ...
                completion: { cvImageBuffer, frameData in
                    ...
                    self.addFeatureColor(frameData: frameData)
    ...
    private func processImageFrameData(
        ...
        outputPixelFormat: BNBPixelFormat = .rgba,
        completion: @escaping (UIImage?) -> Void
    ) {
        ...
        renderQueue.async { [weak self] in
            ...
            self?.activateContext()
            self?.addFeatureColor(frameData: inputFrameData)
            ...
            let processedImage = UIImage(
                rgbaDataNoCopy: outputData as NSData,
    ...
    public func processImageData(
        ...
        renderQueue.async { [weak self] in
            autoreleasepool {
                ...
                self?.addFeatureColor(frameData: fd)
                ...
                let procImage = UIImage(
                    rgbaDataNoCopy: processed as NSData,
                    width: width,
                    height: height
                )
...
private extension BanubaSdkManager {
    func initPlayer(configuration: EffectPlayerConfiguration) {
        ...
        let faceSearchMode = BNBUtilityManager.getHardwareClass() == .low ?
            BNBFaceSearchMode.goodForFirstFace : BNBFaceSearchMode.good
    ...
    func addFeatureColor(frameData: BNBFrameData?) {
        if let color = self.featureColor {
            frameData?.addFeatureColor(color[0], g: color[1], b: color[2], a: color[3])
        }
    }
}
