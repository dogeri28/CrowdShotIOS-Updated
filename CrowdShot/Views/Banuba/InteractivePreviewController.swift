//
//  InteractivePreviewController.swift
//  CrowdShot
//
    ...
    @IBAction func didTap(gesture:UIGestureRecognizer) {
        if gesture.state == UIGestureRecognizer.State.ended {
            ...
            coalesce(timeout: 0.350) { [weak self] in
                self?.sdkManager?.processImageData(self!.srcImage!, params: BNBProcessImageParams(acneProcessing: false, acneUserAreas:self?.rects, faceDataJsonPath: nil)) { [weak self] (image) in
...
extension InteractivePreviewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if indexPath.section == 0 {
            _ = sdkManager?.loadEffect(externalEffects[indexPath.row], synchronous: true)
            processImage()
        }
        _ = sdkManager?.loadEffect(effects[indexPath.row], synchronous: true)
        processImage()
    }
}
